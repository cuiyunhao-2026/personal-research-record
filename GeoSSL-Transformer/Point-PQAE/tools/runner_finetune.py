...
def run_net(args, config, train_writer=None, val_writer=None):
    assert config.model.NAME == 'PointTransformer', 'please implement other PointTransformer model before you run this, PointTransformer only now'
    ...
    if args.resume:
        ...
        if args.ckpts is not None:
            ...
            base_model.load_model_from_ckpt(args.ckpts, model_prefix=args.model_prefix)         
        else:
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        if epoch % args.val_freq == 0 and epoch != 0:
            ...
            if args.vote:   # can be tuned mannually NOTE ScanObjectNN should not be voted during testing in Point-PQAE
                acc1 = 88
                acc2 = 88   
                    
                if metrics.acc > acc1 or (better and metrics.acc > acc2):    
                    metrics_vote = validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, best_metrics_vote=best_metrics_vote, logger=logger)
