...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    # build dataset
    if args.whole:
        config.dataset.train.others.whole = bool(args.whole)    
        print_log("config.dataset.train.others.whole: {}".format(config.dataset.train.others.whole), logger=logger)
    else:
        print_log("config.dataset.train.others.whole: False", logger=logger)
    train_sampler, train_dataloader = builder.dataset_builder(args, config.dataset.train) 
    if config.validate != "none":       # not used in PQAE which is originated from ReCon 
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        if args.distributed:
            train_sampler.set_epoch(epoch)  # NOTE
        base_model.train()
        ...
        idx = 0
        for taxonomy_ids, model_ids, pc in train_dataloader:
            num_iter += 1
            idx += 1
            ...
            tmp_losses = base_model(points)
            assert len(tmp_losses) in [1, 2, 3, 4, 5, 6]
            
            pq_loss = 0
            if 'pq_loss1' in tmp_losses:
                if 'pq_loss2' in tmp_losses:
                    pq_loss = tmp_losses['pq_loss1'] + tmp_losses['pq_loss2']
                else:
                    pq_loss = tmp_losses['pq_loss1']
                
            loss = sum(tmp_losses.values())
            try:
                loss.backward()
                # print("Using one GPU")
            except:
...
def validate(extra_train_dataloader, extra_test_dataloader, epoch, val_writer, args, config, logger=None):
    # just for reference which is not used in Point-PQAE
    # more detail about this function please refer to the code base ReCon
