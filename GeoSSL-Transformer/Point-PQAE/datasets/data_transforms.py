...
class PointcloudRotate(object):
    def __call__(self, pc, angles_ret_needed=False):
        bsize = pc.size()[0]
        rotation_angles = torch.zeros(bsize)
        for i in range(bsize):
            rotation_angle = np.random.uniform() * 2 * np.pi
            rotation_angles[i] = rotation_angle
        ...
        if angles_ret_needed:
            return rotation_angles.to(pc.device), pc
        return pc
...
class RandomHorizontalFlip(object):
  ...
  def __call__(self, coords):
    raise ValueError    # problem exist?
    bsize = coords.size()[0]
    for i in range(bsize):
